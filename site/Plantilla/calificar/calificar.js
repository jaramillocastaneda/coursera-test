...
app.controller('calificarController',['$scope', '$routeParams',  function($scope, $routeParams) {
	...
	$scope.productos = [
      {codigo:2, nombre:'BASE DE DATOS'},
      {codigo:4, nombre:'DESARROLLO'},
      {codigo:3, nombre:'GESTIÓN DE INFORMACIÓN'},
      {codigo:1, nombre:'INTEGRACION'},
      {codigo:5, nombre:'SERVIDORES DE APLICACIONES'},
      {codigo:6, nombre:'SISTEMA OPERATIVO'}
  ];
	...
	$scope.oportunidad = {
		cliente: 0,
		nombreCliente:'ALMACENES EXITO',
		ciudad: 0,
		nombreCiudad: 'MEDELLIN',
		contacto: 'PEDRO PEREZ',
		email: 'pedroperez@seti.com.co',
		telefono: '12345678',
		cargo: 'CARGO CLIENTE 1',
    usuario: 'JORGE VALBUENA',
		comentario: 'How you style your comments can have a huge impact on the overall perception visitors have of your blog'
	};
	$scope.preventa = {
		numero: 1,
		nombre: '',
  ...
  $scope.ciudadSeleccionada = {codigo:-1, nombre:' '};
	...
	$scope.iniciar = function (index) {
      if ($scope.preventa.numero == 0) {
        $scope.tabActivo = 1;
      }
      else {
        $scope.tabActivo = 2;
...
$scope.mostrarOportunidad = false
	$scope.showOportunidad = function (index) {
		$scope.mostrarOportunidad = !$scope.mostrarOportunidad;
    }

	$scope.addCheckList = function() {
     $scope.checkList.splice(0,0,
     {
          producto: 0,
          fecha:'',
          descripcion: '',
